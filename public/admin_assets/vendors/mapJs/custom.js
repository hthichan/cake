...
var map = new H.Map(document.getElementById("map1"), defaultLayers.normal.map, {
...
var map = new H.Map(document.getElementById("map3"), defaultLayers.normal.map, {
...
var map = new H.Map(document.getElementById("map4"), defaultLayers.normal.map, {
    center: { lat: 27.98805, lng: 86.925 },
    zoom: 10,
    pixelRatio: pixelRatio,
});
var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents(map));
...
var map = new H.Map(document.getElementById("map5"), defaultLayers.normal.map, {
...
function panTheMap(map) {
    ...
    viewPort.startInteraction(
        H.map.render.RenderEngine.InteractionModifiers.COORD
    );
    setInterval(pan, 10);
}
var map = new H.Map(document.getElementById("map6"), defaultLayers.normal.map, {
...
var map = new H.Map(document.getElementById("map7"), defaultLayers.normal.map, {
    center: { lat: 42.3572, lng: -71.0596 },
    zoom: 14,
    pixelRatio: pixelRatio,
});
...
function addVenueLayer(map, platform, renderControls) {
    var customVenueLayer = platform
        .getVenueService()
        .createTileLayer({ onSpaceCreated: onSpaceCreated });
...
function onSpaceCreated(space) {
    if (space.getData().preview === "H&M") {
        space.setStyle({ fillColor: "rgba(0,255,0,0.3)" });
    }
}
...
function setUpCustomZooming(map) {
    ...
    var torontoCircle = new H.map.Circle(new H.geo.Point(43.7, -79.4), 75090, {
        style: { fillColor: "rgba(0, 255, 221, 0.66)" },
    });
...
var map = new H.Map(
    document.getElementById("map11"),
    defaultLayers.satellite.map,
    { center: { lat: 0, lng: 0 }, zoom: 1, pixelRatio: pixelRatio }
);
var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents(map));
var ui = H.ui.UI.createDefault(map, defaultLayers);
ui.removeControl("mapsettings");
...
var map = new H.Map(
    ...
    defaultLayers.normal.map,
    { center: { lat: 50, lng: 5 }, zoom: 4, pixelRatio: pixelRatio }
);
var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents(map));
var ui = H.ui.UI.createDefault(map, defaultLayers);
addMarkersToMap(map);
...
var map = new H.Map(
    document.getElementById("map14"),
    defaultLayers.normal.map,
    { center: { lat: 52, lng: 5 }, zoom: 5, pixelRatio: pixelRatio }
);
var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents(map));
var ui = H.ui.UI.createDefault(map, defaultLayers);
...
function addCircleToMap(map) {
    map.addObject(
        new H.map.Circle({ lat: 28.6071, lng: 77.2127 }, 1000, {
            style: {
                strokeColor: "rgba(55, 85, 170, 0.6)",
                lineWidth: 2,
...
var map = new H.Map(
    document.getElementById("map16"),
    defaultLayers.normal.map,
    { center: { lat: 28.6071, lng: 77.2127 }, zoom: 13, pixelRatio: pixelRatio }
);
var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents(map));
var ui = H.ui.UI.createDefault(map, defaultLayers);
addCircleToMap(map);
